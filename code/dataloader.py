...
def process_transactions(df):
    shifted = (df.sort_values([
        'client_id', 'trans_date'
    ]).groupby('client_id')['trans_date'].shift(1)).reset_index()
...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        raw_df['client_id'] = raw_df['client_id'].map(
            {v: k
             for k, v in enumerate(self.idx2client)})

        self.df = process_transactions(raw_df)
        self.df = self.df.sort_values(['client_id', 'trans_date'])

        self.numerical = torch.from_numpy(
            self.df.drop(['client_id', 'small_group'],
                         axis=1).to_numpy()).type(torch.float32)
...
class AgeGroupMLDataset(BaseDataset):
    ...
    def __getitem__(self, idx: int):
        ...
        sn = torch.zeros((self.num_of_subseq, self.subseq_length,
                          n.size(1))).type(torch.float32)
        sc = torch.zeros((self.num_of_subseq, self.subseq_length,
                          c.size(1))).type(torch.int)

        seq_len = n.size(0)
        for i in range(self.num_of_subseq):
            start_index = np.random.randint(0,
                                            seq_len - self.subseq_length + 1)
